    ...
    <article ref={innerRef} {...draggableProps} className={`glass-card p-3 transition-shadow duration-200 ${isDragging ? "scale-[1.015] shadow-2xl ring-1 ring-primary/30" : ""}`}>
      <div className="flex justify-between gap-2">
        <div className="flex min-w-0 gap-1">
          <button type="button" aria-label={`Reorder ${formula.name}`} title="Drag to reorder" {...dragHandleProps} className="mt-0.5 shrink-0 cursor-grab touch-none rounded p-0.5 text-muted-foreground hover:bg-muted hover:text-foreground active:cursor-grabbing"><GripVertical className="h-5 w-5" /></button>
          <div>
