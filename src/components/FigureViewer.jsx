...
export default function FigureViewer({ url, label = 'Figure' }) {
  const [src, setSrc] = useState(''); const [state, setState] = useState('loading'); const [open, setOpen] = useState(false); const [zoom, setZoom] = useState(1);
  useEffect(() => { if (!url) return; const controller = new AbortController(); setSrc(''); setState('loading'); setOpen(false); setZoom(1); fetch('/api/image/resolve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url }), signal: controller.signal }).then((r) => r.json()).then((data) => { if (!data.success) throw new Error(); setSrc(data.resolvedUrl); }).catch(() => setState('error')); return () => controller.abort(); }, [url]);
  if (!url) return null;
  return <><figure className="figure-frame relative overflow-hidden rounded-[10px] border border-border/70 bg-muted/30"><div className="relative flex min-h-28 items-center justify-center">{state === 'loading' && <><div className="absolute inset-0 animate-pulse bg-muted/50" /><Loader2 className="relative h-5 w-5 animate-spin text-muted-foreground" /></>}{state === 'error' ? <div className="flex min-h-28 flex-col items-center justify-center gap-2 text-center text-sm text-muted-foreground"><ImageOff className="h-5 w-5" /><span>Unable to load image.</span></div> : <img src={src} alt={label} loading="lazy" decoding="async" onLoad={() => setState('loaded')} onError={() => setState('error')} onClick={() => setOpen(true)} className={`figure-image cursor-zoom-in object-contain ${state === 'loading' ? 'invisible' : ''}`} />}{state === 'loaded' && <button type="button" aria-label="Fullscreen image" onClick={() => setOpen(true)} className="absolute right-2 top-2 rounded bg-background/90 p-1.5 shadow"><Maximize2 className="h-4 w-4" /></button>}</div></figure><Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-5xl"><DialogHeader><DialogTitle>{label}</DialogTitle></DialogHeader><div className="relative max-h-[78vh] overflow-auto bg-muted/30"><div className="sticky top-2 z-10 flex justify-end gap-1 p-2"><button type="button" onClick={() => setZoom((v) => Math.max(.5, v - .25))} className="rounded bg-background p-2"><Minus className="h-4 w-4" /></button><button type="button" onClick={() => setZoom((v) => Math.min(3, v + .25))} className="rounded bg-background p-2"><Plus className="h-4 w-4" /></button><button type="button" onClick={() => document.fullscreenElement ? document.exitFullscreen() : document.querySelector('.figure-fullscreen')?.requestFullscreen()} className="rounded bg-background p-2"><Expand className="h-4 w-4" /></button></div><img src={src} alt={label} style={{ transform: `scale(${zoom})` }} className="figure-fullscreen mx-auto max-h-[75vh] w-full object-contain" /></div></DialogContent></Dialog></>;
>>>>>>> 71bc133 (stresss)
}
