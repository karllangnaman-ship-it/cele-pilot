      ...
      >
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <Collapsible open={namesOpen} onOpenChange={setNamesOpen} className="rounded-md border"><CollapsibleTrigger className="flex w-full items-center gap-2 p-3 text-left font-medium"><ChevronDown className={`h-4 w-4 transition-transform ${namesOpen ? 'rotate-180' : ''}`} />Formula Name(s) <span className="text-muted-foreground">(Optional)</span></CollapsibleTrigger><CollapsibleContent className="space-y-3 border-t p-3">{aiConfig.formulaNames.map((value, index) => <div key={index}><Label>Formula Name {index + 1}</Label><div className="flex gap-2"><Input value={value} onChange={(event) => setAiConfig((current) => ({ ...current, formulaNames: current.formulaNames.map((item, i) => i === index ? event.target.value : item) }))} placeholder="Stress" />{aiConfig.formulaNames.length > 1 && <Button type="button" variant="ghost" size="icon" aria-label="Remove formula name" onClick={() => setAiConfig((current) => ({ ...current, formulaNames: current.formulaNames.filter((_, i) => i !== index) }))}><X className="h-4 w-4" /></Button>}</div></div>)}{aiConfig.formulaNames.length < 5 && <Button type="button" variant="ghost" className="px-0" onClick={() => setAiConfig((current) => ({ ...current, formulaNames: [...current.formulaNames, ''] }))}><Plus className="mr-1 h-4 w-4" />Add Another</Button>}</CollapsibleContent></Collapsible>
