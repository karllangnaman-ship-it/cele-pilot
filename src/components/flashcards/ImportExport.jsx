...
const configs = {
  ...
  question: {
    ...
    columns: ['Situation ID', 'Situation Title', 'Situation Description', 'Figure Label', 'Image URL', 'Subject', 'Topic', 'Question Number', 'Question', 'Option A', 'Option B', 'Option C', 'Option D', 'Correct Answer', 'Explanation', 'Difficulty', 'Tags'],
    sample: [{ 'Situation ID': 'SIT-001', 'Situation Title': 'Simply Supported Beam', 'Situation Description': 'A simply supported beam carries the loading shown in Figure 1.', 'Figure Label': 'Figure 1', 'Image URL': 'https://example.com/beam-diagram.png', Subject: 'PSAD', Topic: 'Mechanics of Materials', 'Question Number': '1', Question: 'What is the reaction at A?', 'Option A': '10 kN', 'Option B': '15 kN', 'Option C': '20 kN', 'Option D': '25 kN', 'Correct Answer': 'A', Explanation: 'Apply equilibrium.', Difficulty: 'medium', Tags: 'beam, statics' }],
    description: 'Import standalone or grouped board-exam questions with remote figures.',
    map: (row) => mapSpreadsheetRow(row, 'question'), validate: (item) => spreadsheetValidationReasons(item, 'question'),
    export: (item, situations = []) => { const situation = situations.find((entry) => entry.id === item.situationId); return { 'Situation ID': situation?.externalId || item.situationKey || '', 'Situation Title': situation?.title || item.situationTitle || '', 'Situation Description': situation?.description || item.situationDescription || '', 'Figure Label': situation?.figureLabel || item.figureLabel || '', 'Image URL': situation?.imageUrl || item.imageUrl || '', Subject: item.subject || '', Topic: item.topic || '', 'Question Number': item.questionNumber || '', Question: item.question || '', 'Option A': item.choices?.[0] || '', 'Option B': item.choices?.[1] || '', 'Option C': item.choices?.[2] || '', 'Option D': item.choices?.[3] || '', 'Correct Answer': item.correctAnswer || '', Explanation: item.explanation || '', Difficulty: item.difficulty || '', Tags: Array.isArray(item.tags) ? item.tags.join(', ') : item.tags || '' }; },
    preview: ['Situation ID', 'Question Number', 'Question', 'Option A', 'Option B', 'Option C', 'Option D', 'Correct Answer'], importEntries: importQuestionEntries,
  },
};
...
async function importQuestionEntries({ entries, userId, onProgress }) {
  ...
  entries.forEach((entry) => { const key = entry.item.situationKey?.trim(); if (key) groups.set(key, [...(groups.get(key) || []), entry]); else standalone.push(entry); });
  const created = []; const failedRows = []; let completed = 0;
  for (const [situationKey, group] of groups) {
    const first = group[0].item;
    try {
      const situation = await firebaseApi.entities.Situation.create({ user_id: userId, externalId: situationKey, title: first.situationTitle || situationKey, description: first.situationDescription || '', imageUrl: first.imageUrl || null, figureLabel: first.figureLabel || '', sourceType: 'spreadsheet' });
      const questions = await firebaseApi.entities.Question.bulkCreate(group.map(({ item }) => ({ ...item, user_id: userId, situationId: situation.id, imageUrl: null, figureLabel: '', situationTitle: '', situationDescription: '' })));
      created.push(...questions); completed += group.length;
    } catch (error) { group.forEach((entry) => failedRows.push({ ...entry, reasons: [`Firestore write failed: ${error.message || 'Unknown error'}`] })); }
    onProgress?.({ phase: 'Saving to Firestore', progress: 50 + Math.round((completed / Math.max(entries.length, 1)) * 50), total: entries.length, imported: created.length });
  }
  if (standalone.length) { const result = await batchWriteItems({ entries: standalone, entity: firebaseApi.entities.Question, userId, total: entries.length, onProgress }); created.push(...result.created); failedRows.push(...result.failedRows); }
  return { created, failedRows };
}

export default function ImportExport({ user, cards = [], relations = [], onImported, type = 'flashcard' }) {
  ...
  const rowsFor = (items) => items.map((item) => config.export(item, relations));
  ...
  const confirmImport = async () => { if (!preview.length || !user) return; setStatus({ phase: 'Saving to Firestore', progress: 50, total: preview.length });
    try { const { created, failedRows } = await (config.importEntries ? config.importEntries({ entries: preview, userId: user.id, onProgress: setStatus }) : batchWriteItems({ entries: preview, entity: config.entity, userId: user.id, total: preview.length, onProgress: setStatus })); onImported(created); toast({ title: `${created.length} ${config.title.toLowerCase()} imported!`, description: failedRows.length ? `${failedRows.length} rows could not be saved.` : 'Saved directly to Firestore.' }); setPreview([]); setOpen(false); setStatus(null); }
