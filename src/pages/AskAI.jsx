...
import { normalizeGeminiResponse } from '@/lib/GeminiContentParser';
...
const RichMarkdownRenderer = lazy(() => import('@/components/RichMarkdownRenderer'));

class ResponseRendererBoundary extends Component {
  state = { failed: false };
  static getDerivedStateFromError() { return { failed: true }; }
  componentDidCatch(error) { console.error('[Ask AI] rich renderer failed to load', error); }
  render() { return this.state.failed ? <div className="whitespace-pre-wrap">{this.props.content || 'No response.'}</div> : this.props.children; }
}

function AssistantResponse({ content, pending }) {
  const fallback = <div className="whitespace-pre-wrap">{content || (pending ? '…' : 'No response.')}</div>;
  return <ResponseRendererBoundary content={content}><Suspense fallback={fallback}><RichMarkdownRenderer content={content} pending={pending} /></Suspense></ResponseRendererBoundary>;
}
...
function Message({ item, onDelete, onRegenerate, onRetry, onEdit }) {
  ...
  return <article className={`group flex gap-3 ${item.role === 'user' ? 'justify-end' : ''}`}>
    ...
    <div className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm shadow-sm ${item.role === 'user' ? 'bg-primary text-primary-foreground' : 'border border-border/60 bg-card'}`}>
      ...
      {item.role === 'assistant' ? <AssistantResponse content={content} pending={item.pending} /> : <LatexText value={content} />}
