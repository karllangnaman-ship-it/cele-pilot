...
export const blankQuestion = () => ({ subject: '', topic: '', difficulty: 'medium', question: '', choices: ['', '', '', ''], correctAnswer: '', explanation: '', formulaReferences: '', tags: '', boardYear: '', source: '', sourceType: 'manual', visibility: 'private', confidence: 1 });
...
export function mapSpreadsheetRow(row, type) {
  ...
  return normalizeItem({ subject: first(values, 'subject'), topic: first(values, 'topic'), difficulty: first(values, 'difficulty') || 'medium', question: first(values, 'question'), choices: [first(values, 'choicea', 'a', 'optiona'), first(values, 'choiceb', 'b', 'optionb'), first(values, 'choicec', 'c', 'optionc'), first(values, 'choiced', 'd', 'optiond')].map((value, index) => value || splitValues(first(values, 'choices'))[index] || ''), correctAnswer: first(values, 'correctanswer', 'answer'), explanation: first(values, 'explanation'), formulaReferences: first(values, 'formulareferences', 'formulareference'), tags: splitValues(first(values, 'tags')), boardYear: first(values, 'boardyear'), source: first(values, 'source') }, type, 'spreadsheet');
}
